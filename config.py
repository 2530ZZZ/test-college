...
# 原理：均值 83/min（4800/60）的 2.4 倍余量；200/min 对 200 worker
# 足够（仓库处理大多走零 API 的 clone/raw 路径）。
# 默认值：200。取值范围 100-300。
API_MAX_PER_MINUTE = 200

# API 速率门暂停阈值（次/分钟）。
# 作用：最近 60s core 放行数 ≥ 此值 → Worker 暂停取新任务（削峰）。
# 原理：160 = 200 的 80%，接近上限前提前收手，在途请求消化后速率回落。
# 默认值：160。取值范围 100-{API_MAX_PER_MINUTE}。
API_PAUSE_AT_RATE = 160

# API 速率门恢复阈值（次/分钟）。
# 作用：core 速率 ≤ 此值 → Worker 恢复取任务。
# 原理：100 = 200 的 50%，滞回带（100-160）防止阈值附近反复抖动。
# 默认值：100。取值范围 50-{API_PAUSE_AT_RATE}。
API_RESUME_AT_RATE = 100
