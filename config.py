...
# （081XX 曾试验 10800=3h 配合降频——用户决定不改运行策略，还原 19800。）
...
# 丢进队列，下载线程从队列取。队列放链接不占内存（20 万条 ≈ 20MB）。
# 队列满 → worker 阻塞等待（可中断，收尾能退出）——背压反推，防止
# 无限积压。先进先出（queue.Queue 天然 FIFO，旧文件先下载，防旧数据滞留）。
# 081XX：10 万 → 20 万——08261 实测背压期间"在途仓库批量入队"瞬间撑满
# 10 万（qsize 显示 100749 超上限），增大后突发入队有缓冲，在途 worker
# 不卡入队（设计确认：背压不挡入队，此队列是突发缓冲）。
DOWNLOAD_QUEUE_SIZE = 500000
...
# 081XX：30→40——08311 实测下载吞吐被节流锁死在 ~28/s（1688/60s），
# 是系统总吞吐上限；40/s 仍有 60s 失败窗口降级兜底（DOWNLOAD_STALL_
# THRESHOLD 触发连接减半），真触发 CDN 限速会自动退回 20/s，先试验。
...
MAX_RAW_DOWNLOAD_CONNECTS_PER_SEC = 40
